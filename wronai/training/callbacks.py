    ...
    def on_train_end(
        ...
        **kwargs,
        ...
        if self.memory_history:
            ...
            avg_memory = sum(
                item["memory_percent"] for item in self.memory_history
            ) / len(self.memory_history)

            logger.info(
                f"Memory usage summary - Max: {max_memory:.1f}%, Average: {avg_memory:.1f}%"
            )
            ...
            if tokenizer and hasattr(model, "generate"):
                ...
                for prompt in self.polish_prompts[:3]:  # Test with first 3 prompts
                    try:
                        ...
                        logger.warning(
                            f"Polish evaluation failed for prompt '{prompt}': {e}"
                        )

                if results:
                    avg_length = sum(r["length"] for r in results) / len(results)
                    logger.info(
                        f"Polish generation test - Average length: {avg_length:.1f} words"
                    )
        ...
        **kwargs,
        ...
        if self.last_step_time is not None:
            ...
            if state.global_step % 50 == 0:
                avg_step_time = sum(self.step_times[-50:]) / min(
                    50, len(self.step_times)
                )
                avg_tokens_per_sec = sum(self.tokens_per_second_history[-50:]) / min(
                    50, len(self.tokens_per_second_history)
                )
...
class LossMonitorCallback(TrainerCallback):
    ...
    def __init__(self, loss_spike_threshold: float = 2.0, loss_window_size: int = 10):
        ...
        **kwargs,
        ...
        if logs and "train_loss" in logs:
            ...
            if state.global_step % 100 == 0 and len(self.loss_history) >= 20:
                ...
                older_avg = (
                    sum(self.loss_history[-40:-20]) / 20
...
class CheckpointCleanupCallback(TrainerCallback):
    ...
    def _cleanup_old_checkpoints(self, args: TrainingArguments):
        ...
        sorted_checkpoints = sorted(
            self.checkpoint_scores,
            key=lambda x: (
                x["best_metric"] if x["best_metric"] is not None else float("inf")
            ),
        )
                ...
                self.wandb.log(
                    {
                        ...
                        "model/trainable_parameters": getattr(
                            model, "get_trainable_parameter_count", lambda: 0
                        )(),
                    }
                )
        ...
        try:
            ...
            if memory_info:
                enhanced_logs.update(
                    {
                        "system/gpu_memory_percent": memory_info.get(
                            "memory_percent", 0
                        ),
                        "system/gpu_memory_used_gb": memory_info.get(
                            "memory_used_gb", 0
                        ),
                    }
                )
