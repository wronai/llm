...
class PolishDataProcessor:
    ...
    def download_oscar_polish(self) -> str:
        ...
        try:
            ...
            dataset = load_dataset("oscar-corpus/OSCAR-2301", "pl", split="train", streaming=True)
            
            oscar_path = self.output_dir / "oscar_polish.jsonl"
            
            with open(oscar_path, 'w', encoding='utf-8') as f:
                count = 0
                for item in tqdm(dataset.take(100000), desc="Processing OSCAR"):  # Take 100k samples
    ...
    def create_instruction_dataset(self) -> str:
        ...
        qa_pairs = [
            ("Co to jest demokratacja?", "Demokracja to ustrój polityczny, w którym władza należy do narodu i jest sprawowana przez wybrane przez niego organy."),
            ("Kiedy Polska odzyskała niepodległość?", "Polska odzyskała niepodległość 11 listopada 1918 roku, po 123 latach zaborów."),
            ("Jak nazywa się stolica Polski?", "Stolicą Polski jest Warszawa."),
            ("Ile województw ma Polska?", "Polska ma 16 województw."),
            ("Jakie morze obmywa Polskę?", "Polska jest obmywana przez Morze Bałtyckie."),
            ("Kto napisał 'Pana Tadeusza'?", "Autorem 'Pana Tadeusza' jest Adam Mickiewicz."),
    ...
    def is_good_quality_text(self, text: str) -> bool:
        ...
        non_alpha_ratio = len([c for c in text if not c.isalpha() and not c.isspace()]) / len(text)
...
def main():
    ...
    parser.add_argument(
        "--download-wikipedia",
        action="store_true",
        help="Download Polish Wikipedia"
    )
    parser.add_argument(
        "--download-oscar",
        action="store_true",
        help="Download OSCAR Polish subset"
    )
    parser.add_argument(
        "--create-instructions",
        action="store_true",
        help="Create instruction dataset"
    )
    parser.add_argument(
        "--combine-all",
        action="store_true",
